...
while True:
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        if True in matches:
            ...
            # Check if name already exists in MySQL table
            mycursor = mydb.cursor()
            mycursor.execute("SELECT * FROM attendance WHERE name = %s", (name,))
            result = mycursor.fetchone()

            # If name does not exist, add it to MySQL table with status 'present'
            if result is None:
                mycursor.execute("INSERT INTO attendance (name, status) VALUES (%s, %s)", (name, 'present'))
                mydb.commit()
